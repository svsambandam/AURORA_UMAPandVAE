...
class VAE(object):
    ...
    def create_net(self):
        ...
        with tf.variable_scope('encoder') as vs:
            ...
            self.mu = FullConnected(self.layers[-1], self.layers[-1].get_shape().as_list()[1], self.latent_dim, activation='identity', name = "mu").output()
            self.std = FullConnected(self.layers[-1], self.layers[-1].get_shape().as_list()[1], self.latent_dim, activation='identity', name = "std").output()
            self.layers.append(self.mu)
            self.layers.append(self.std)
            eps = tf.random_normal(shape=tf.shape(self.std), mean=0, stddev=1, dtype=tf.float32)
            self.z = self.mu + tf.sqrt(tf.exp(self.std)) * eps
            self.layers.append(self.z)
    ...
    def create_loss(self, ):
        recon_loss = math_ops.squared_difference(self.x,self.decoded, name = "recon_loss")
        latent_loss = -0.5 * tf.reduce_sum(1 + self.std - tf.square(self.mu) - tf.exp(self.std), axis=1)
        self.loss = tf.reduce_mean(recon_loss + latent_loss, 0, name = "loss") 
